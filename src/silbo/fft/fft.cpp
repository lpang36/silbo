...
static cv::Mat FFT::fft(const std::string& path, , const Config& config) {
    AudioFile<double> file;
    file.load(path);

    const size_t cols = config.block_size;
    const size_t rows = file.getNumSamplesPerChannel() / cols;
    auto samples = cv::Mat{rows, config.block_size, cv::CV_32F};
    for (size_t i = 0; i < file.getNumSamplesPerChannel(); ++i) {
        ...
        samples.at<float>(i / cols, i % cols) = audioFile.samples[0][i];
    }

    // omitting hanning window for now
  	// http://docs.opencv.org/doc/tutorials/core/discrete_fourier_transform/discrete_fourier_transform.html
    cv::Mat planes[] = {cv::Mat::zeros(1, cols, cv::CV_32F), cv::Mat::zeros(1, cols, cv::CV_32F)};
    cv::Mat complex;
